...
class PineconeDocumentStore:
    ...
    def get_index_stats(self):
        ...
        count = (
            stats["namespaces"][""]["vector_count"]
            if stats["namespaces"].get("")
            else 0
        )
        logger.info(
    ...
    def _index_connection_exists(
        self, index: str, create: bool = False
    ...
    def _add_type_metadata_filter(
        ...
        if type_value:
            ...
            if (
                AND_OPERATOR not in filters
                and TYPE_METADATA_FIELD not in filters
            ):
            ...
            filters_content = (
                filters[AND_OPERATOR] if AND_OPERATOR in filters else filters
            )
            if TYPE_METADATA_FIELD in filters_content:  # type: ignore
                ...
                if isinstance(current_type_filter, str):
                    ...
                    if EQ_OPERATOR in current_type_filter:
                        ...
                        type_values.update(
                            set(current_type_filter[IN_OPERATOR])
                        )
    ...
    def _get_default_type_metadata(
        self, index: Optional[str], namespace: Optional[str] = None
    ) -> str:
        """
    ...
    def get_document_count(
        ...
        if not type_metadata:
            ...
            filters = self._add_type_metadata_filter(
                filters, type_value=type_metadata
            )

        pinecone_syntax_filter = (
            LogicalFilterClause.parse(filters).convert_to_pinecone()
            if filters
            else None
        )
        return self._get_vector_count(
            index, filters=pinecone_syntax_filter, namespace=namespace
        )
    ...
    def get_embedding_count(
        ...
        if filters:
            raise NotImplementedError(
                "Filters are not supported for get_embedding_count in PineconeDocumentStore"
            )

        index = self._index(index)
        self._index_connection_exists(index)

        pinecone_filters = self._meta_for_pinecone(
            {TYPE_METADATA_FIELD: DOCUMENT_WITH_EMBEDDING}
        )
        return self._get_vector_count(
            index, filters=pinecone_filters, namespace=namespace
        )

    def _meta_for_pinecone(
        self, meta: Dict[str, Any], parent_key: str = "", labels: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    def _pinecone_meta_format(
        self, meta: Dict[str, Any], labels: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    def count_documents(self) -> int:
        ...
        """
        count = (
            self.index_stats["namespaces"][""]["vector_count"]
            if self.index_stats["namespaces"].get("")
            else 0
        )
        return count
    ...
    def write_documents(
        ...
        type_metadata = (
            ...
            if add_vectors
            else DOCUMENT_WITHOUT_EMBEDDING
        )

        if not add_vectors:
            # To store documents in Pinecone, we use dummy embeddings (to be replaced with real embeddings later)
            embeddings_to_index = np.zeros(
                (self.batch_size, self.embedding_dim), dtype="float32"
            )
            # Convert embeddings to list objects
            embeddings = [
                embed.tolist() if embed is not None else None
                for embed in embeddings_to_index
            ]
        ...
        with tqdm(
            ...
            for i in range(0, len(documents), self.batch_size):
                ...
                if duplicate_documents in ["skip", "fail"]:
                    ...
                    if len(ids) != len(set(ids)):
                        if duplicate_documents == "skip":
                            ...
                            # Otherwise, we raise an error
                            raise DuplicateDocumentError(
                                f"Duplicate document IDs found in batch: {ids}"
                            )
                metadata = [
                    self._meta_for_pinecone(
                        {
                ...
                self.pinecone_indexes[index].upsert(
                    vectors=data_to_write_to_pinecone, namespace=self.namespace
                )
                # Add IDs to ID list
    ...
    def query_by_embedding(
        ...
        scores_for_vector_ids: Dict[str, float] = {
            str(v_id): s for v_id, s in zip(vector_id_matrix, score_matrix)
        }
        return_documents = []
    ...
    def _get_documents_by_meta(
        ...
        for _id, meta in zip(ids, metadata):
            ...
            doc = Document(
                id=_id, content=content, content_type=content_type, meta=meta
            )
        ...
        if return_embedding:
            if values is None:
                ...
                for doc in documents:
                    self._attach_embedding_to_document(
    ...
    def get_documents_by_id(
        ...
        for i in range(0, len(ids), batch_size):
            ...
            result = self.pinecone_indexes[index].fetch(
                ids=id_batch, namespace=namespace
            )
            ...
            for _id in result["vectors"]:
                ...
                if (
                    not include_type_metadata
                    and TYPE_METADATA_FIELD in metadata
                ):
    ...
    def delete_documents(self, document_ids: List[str]) -> None:
        ...
        if document_ids is None:
            ...
            self.pinecone_indexes[index].delete(
                ids=document_ids, namespace=self.namespace
            )
